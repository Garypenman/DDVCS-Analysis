...
double MakeEvent(double Ebeam, double Pbeam, double Q2, double y, double Qp2, double t, double phi, double costhetaL, double phiL){
  ...
  auto photo_flux = (alpha/(2.0*M_PI*Q2)) * (1.0 + (1.0-y)*(1.0-y)/y - 2.0*(1.0-y)*Q2min/(y*Q2) ) * nu/(ebeam_tar.E()*xbj);
  //photo_flux = (alpha/(4.0*M_PI*M*ebeam_tar.E())) * (1/(y*y) -1 - (2*me*me/Q2) );
  ...
  hCosTheta->Fill(cos(thetaL), d4sigma);
  hPhi->Fill(phiL, d4sigma);
...
kinsetup jlab12{10.6, M, 0.0, 1.0, 0.15, 5.0, 2.25, 9.0, -0.8, -0.1, 0.0, 2*M_PI, 0.0, 2*M_PI, 0.1-M_PI, M_PI-0.1, M_PI/4.0, M_PI*3.0/4.0};

kinsetup jlab12_fulltheta{10.6, M, 0.0, 1.0, 0.15, 5.0, 2.25, 9.0, -0.8, -0.1, 0.0, 2*M_PI, 0.0, 2*M_PI, -M_PI, M_PI, 0, M_PI};

kinsetup jlab22{22, M, 0.0, 1.0, 0.15, 5.0, 2.25, 9.0, -0.8, -0.1, 0.0, 2*M_PI, 0.0, 2*M_PI, 0.1-M_PI, M_PI-0.1, M_PI/4.0, M_PI*3.0/4.0};
kinsetup eic5x41{5, 41, 0.0, 1.0, 0.15, 5.0, 2.25, 9.0, -1.0, -0.05, 0.0, 2*M_PI, 0.0, 2*M_PI, 0.1, 2*M_PI-0.1, M_PI/4.0, M_PI*3.0/4.0};
kinsetup eic10x100{10, 100, 0.0, 1.0, 0.15, 5.0, 2.25, 9.0, -1.0, -0.05, 0.0, 2*M_PI, 0.0, 2*M_PI, 0.1, 2*M_PI-0.1, M_PI/4.0, M_PI*3.0/4.0};

kinsetup MYeic18x275{18, 275, 0.0, 1.0, 0.0, 10.0, 2.0, 18.0, -2.0, 0.0, 0.0, 2*M_PI, 0.0, 2*M_PI, -M_PI, M_PI, 0, M_PI};

void IntegrateBH(kinsetup kin=MYeic18x275){
  ...
  hCosTheta = new TH1D("hCosTheta","CosTheta Generated By Integrator;cos(#theta_{l})",100,-1,1);
  hPhi = new TH1D("hPhi","Phi Generated By Integrator; phi_{l} [rad]",100,-M_PI,M_PI);
